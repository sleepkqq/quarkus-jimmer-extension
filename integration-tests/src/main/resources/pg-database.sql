...
create table book_store(
    id bigint generated by default as identity (start with 100 increment by 1) not null primary key,
...
alter table book_store add constraint business_key_book_store unique(name);

create table alias(
    id bigint generated by default as identity (start with 100 increment by 1) not null primary key,
    value varchar(50) not null,
    type varchar(20) not null,
    constraint business_key_alias unique(value)
);

insert into alias(id, value, type) values (1, 'reserved', 'TEAM');
insert into alias(id, value, type) values (2, 'available', 'CONSUMER');
...
alter table book add constraint business_key_book unique(name, edition);
alter table book add constraint fk_book__book_store foreign key(store_id) references book_store(id);
...
alter table author add constraint business_key_author unique(first_name, last_name);
alter table author add constraint ck_author_gender check (gender in ('M', 'F'));

create table book_author_mapping(
    book_id bigint not null,
    author_id bigint not null
);
alter table book_author_mapping add constraint pk_book_author_mapping primary key(book_id, author_id);
alter table book_author_mapping add constraint fk_book_author_mapping__book foreign key(book_id) references book(id) on delete cascade;
alter table book_author_mapping add constraint fk_book_author_mapping__author foreign key(author_id) references author(id) on delete cascade;
...
create table tree_node(
    node_id bigint generated by default as identity (start with 100 increment by 1) not null primary key,
    name varchar(20) not null,
    parent_id bigint,
    created_time timestamp not null,
    modified_time timestamp not null
);
alter table tree_node add constraint business_key_tree_node unique(parent_id, name);
alter table tree_node add constraint fk_tree_node__parent foreign key(parent_id) references tree_node(node_id);

insert into tree_node(node_id, name, parent_id, created_time, modified_time) values
